...
exports.requestGist = requestGist;
exports.extractId = extractId;
exports.renderGist = renderGist;
function loadGist(id) {
  return window.fetch('https://api.github.com/gists/' + id + '?' + Math.random()).then(function (res) {
    return res.json();
  });
}

function requestGist(id) {
  return loadGist(id).then(function (content) {
    var html;
    var js;
    if (!content.files) {
      return null;
...
function tryGistFromUrl() {
  ...
  if (gistId) {
    return (0, _gist.requestGist)(gistId).then(function (content) {
      ...
      (0, _gist.renderGist)(content.html, content.js);
      return true;
    });
  } else {
...
tryGistFromUrl().then(function (isSuccess) {
  if (!isSuccess) {
    (0, _ui.render)(document.getElementById('root'));
  }
});

},{"./gist":2,"./ui":4}],4:[function(_dereq_,module,exports){
'use strict';

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.createWelcomeDialog = createWelcomeDialog;
...
var html = diff.html,
    innerHTML = diff.innerHTML;


function onWelcomeFormSubmit(event) {
  event.preventDefault();
  var urlInputEl = event.currentTarget.querySelector('input[name="url"]');
  var gistId = (0, _gist.extractId)(urlInputEl.value);
  if (!gistId) {
    alert('You need type something into Gist URL input!');
    urlInputEl.focus();
  } else {
    (0, _gist.requestGist)(gistId).then(function (content) {
      if (!content) {
...
function createWelcomeDialog() {
  return diff.createElement("div", [diff.createAttribute("class", 'gist-welcome')], [diff.createElement('#text', null, "\n      "), diff.createElement("div", [diff.createAttribute("class", 'gist-welcome-dialog')], [diff.createElement('#text', null, "\n        "), diff.createElement("p", [], [diff.createElement('#text', null, "You can use this service to run github gists.")]), diff.createElement('#text', null, "\n        "), diff.createElement("p", [], [diff.createElement('#text', null, "Your gist require two files: "), diff.createElement("code", [], [diff.createElement('#text', null, "index.html")]), diff.createElement('#text', null, " or "), diff.createElement("code", [], [diff.createElement('#text', null, "index.js")]), diff.createElement('#text', null, ".")]), diff.createElement('#text', null, "\n        "), diff.createElement("form", [diff.createAttribute("onsubmit", onWelcomeFormSubmit)], [diff.createElement('#text', null, "\n          "), diff.createElement("input", [diff.createAttribute("class", 'gist-welcome-url'), diff.createAttribute("type", 'text'), diff.createAttribute("name", 'url'), diff.createAttribute("placeholder", 'Gist URL or just ID')], []), diff.createElement('#text', null, "\n          "), diff.createElement("input", [diff.createAttribute("type", 'submit'), diff.createAttribute("value", 'Run Gist as HTML page')], []), diff.createElement('#text', null, "\n        ")]), diff.createElement('#text', null, "\n      ")]), diff.createElement('#text', null, "\n    ")]);
}

function createApp() {
  return diff.createElement('', null, createWelcomeDialog());
}
